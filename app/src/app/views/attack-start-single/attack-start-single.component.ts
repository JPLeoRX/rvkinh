...
@Component({
  selector: 'app-attack-start-single',
  templateUrl: './attack-start-single.component.html',
  styleUrls: ['./attack-start-single.component.css']
})
export class AttackStartSingleComponent implements OnInit {
  formGroup: FormGroup;
  ...
  buildForm(): void {
    this.formGroup.addControl('ping_flood_target_ip_address', this._formBuilder.control(''))
    this.formGroup.addControl('syn_flood_target_ip_address', this._formBuilder.control(''))
    this.formGroup.addControl('syn_flood_target_port', this._formBuilder.control(443))
    this.formGroup.addControl('http_flood_target_urls', this._formBuilder.control(''))
    this.formGroup.addControl('parallel_min_requests', this._formBuilder.control(200))
    this.formGroup.addControl('parallel_max_requests', this._formBuilder.control(600))
    this.formGroup.addControl('proxy_type', this._formBuilder.control('tor'))
    this.formGroup.addControl('proxy_ip_change_frequency', this._formBuilder.control(6))
  }
  ...
  onFormSubmit(): void {
    ...
    for (let cluster of this.listOfClusters) {
      goal_akio_by_cluster_id[cluster] = new GoalAkio({
        ping_flood_target_ip_address: this.formGroup.get('ping_flood_target_ip_address')?.value,
        syn_flood_target_ip_address: this.formGroup.get('syn_flood_target_ip_address')?.value,
        syn_flood_target_port: this.formGroup.get('syn_flood_target_port')?.value,
      })
    }

    let goal_haru_by_cluster_id: Record<string, GoalHaru> = {};
    for (let cluster of this.listOfClusters) {
      let http_flood_target_urls = this.formGroup.get('http_flood_target_urls')?.value.split("\n").filter((s: string) => s.length > 0)

      let worker_settings = new WorkerHaruSettings({
        parallel_min_requests: this.formGroup.get('parallel_min_requests')?.value,
        parallel_max_requests: this.formGroup.get('parallel_max_requests')?.value,
        proxy_type: this.formGroup.get('proxy_type')?.value,
        proxy_ip_change_frequency: this.formGroup.get('proxy_ip_change_frequency')?.value,
      });
