...
class JobControllerGetGoal(AbstractJob):
    @autowired
    def __init__(self, storage_target: Autowired(StorageTarget), client_controller: Autowired(ClientController), utils_env: Autowired(UtilsEnv)):
        super().__init__()
    ...
    def job_iteration(self):
        ...
        # Try to get goal
        try:
            goal = self.client_controller.worker_goal_akio(self.controller_url, self.worker_api_key, self.worker)
        except:
            # Sleep and return
