...
def check_single(input: PortscanCheckSingleInput) -> PortscanCheckSingleOutput:
    t1 = time.time()
    target_ip_address, target_port, open = utils_scapy.check_port(input.target_ip_address, input.target_port)
    t2 = time.time()
    t = t2 - t1
...
def check_multiple(input: PortscanCheckMultipleInput) -> PortscanCheckMultipleOutput:
    ...
    for result in executor.map(utils_scapy.check_port, repeat(input.target_ip_address), input.list_of_target_ports):
...
def check_all(input: PortscanCheckAllInput) -> PortscanCheckAllOutput:
    t1 = time.time()
    list_of_target_open_ports = []
    for result in executor.map(utils_scapy.check_port, repeat(input.target_ip_address), TOP_PORTS):
        target_ip_address, target_port, open = result[0], result[1], result[2]
        if open:
            list_of_target_open_ports.append(target_port)
    t2 = time.time()
